...
int main()
{
  Eventful::QueuedBus<E0, E1, E2, E3, E4, E5, E6, E7, E8, E9, E10, E11, E12, E13, E14, E15> bus;
  ...
  // Test performance with function handler
  {
    ...
    int n = bus.process();
    assert(n == static_cast<long long int>(COUNT));
    assert(global == 499999500000ul);
    ...
    std::cout << "Adding " << COUNT << " trivial events with function handler took " << dt1 << " ns (" << dt1/COUNT << " ns per event)" << std::endl;
    std::cout << "Processing " << COUNT << " trivial events with function handler took " << dt2 << " ns (" << dt2/COUNT << " ns per event)" << std::endl;
  }
  ...
  // Test performance with lambda handler
  {
    ...
    int n = bus.process();
    assert(n == static_cast<long long int>(COUNT));
    assert(counter == 499999500000ul);
    auto t2 = std::chrono::steady_clock::now();
    auto dt1 = std::chrono::duration_cast<std::chrono::nanoseconds>(t1 - t0).count();
    auto dt2 = std::chrono::duration_cast<std::chrono::nanoseconds>(t2 - t1).count();
    std::cout << "Adding " << COUNT << " trivial events with lambda handler took " << dt1 << " ns (" << dt1/COUNT << " ns per event)" << std::endl;
    std::cout << "Processing " << COUNT << " trivial events with lambda handler took " << dt2 << " ns (" << dt2/COUNT << " ns per event)" << std::endl;
  }
  ...
  // Test performance with member function handler
  {
    ...
    int n = bus.process();
    assert(n == static_cast<long long int>(COUNT));
    assert(counter.getCount() == 499999500000ul);
    auto t2 = std::chrono::steady_clock::now();
    auto dt1 = std::chrono::duration_cast<std::chrono::nanoseconds>(t1 - t0).count();
    auto dt2 = std::chrono::duration_cast<std::chrono::nanoseconds>(t2 - t1).count();
    std::cout << "Adding " << COUNT << " trivial events with member function handler took " << dt1 << " ns (" << dt1/COUNT << " ns per event)" << std::endl;
    std::cout << "Processing " << COUNT << " trivial events with member function handler took " << dt2 << " ns (" << dt2/COUNT << " ns per event)" << std::endl;
  }
  return 0;
}
